...
def take_header(x):
	...
	return cell, BMAJ,BMIN,BPA,date,freq,epoch,circ_beam, source_name,rms
...
def convolve_difmap_modelfit(files,models,modelfit,bmaj,bmin,bpa,xshift,yshift,mapsize,pixelsize,files2):
	#open difmap and convolve maps with new beam
	from distutils.spawn import find_executable
	difmap_path = find_executable('difmap')
	for i in xrange(0,len(files)): #/usr/local/pgplot.old/uvf_difmap/difmap 
		...
		child_stdin.write('wmap %s\n' %files2[i])
		child_stdin.write('rmod %s\n' %modelfit[i])
		child_stdin.write('modelfit 0 \n')
		child_stdin.write('quit\n')
		p.wait()

def search_modelfit():
	search_string = 'Iteration 00'
	outfile = open('newModelFit.txt', 'w')

	outfile.write('! Center RA: 05 19 49.69000,  Dec: -45 46 44.49998 (2000.0) \n')
	outfile.write('! Established model.\n')
	outfile.write('! Flux (Jy) Radius (mas)  Theta (deg)  Major (mas)  Axial ratio   Phi (deg) T \\ \n')
	outfile.write('! Freq (Hz)     SpecIndex \n')
	...
	with open('difmap.log','r') as infile:
		...
		for line in infile:
			...
			if '!' and '\\' in line:
				...
				s2 = s1[1].split('\\')
				sFile = s2[0]
			if 'e' in line and 'Log' not in line and '\\' not in line:
				s1 = s.split('!')
				sFile = sFile + s1[1]
				outfile.write(sFile)

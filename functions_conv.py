...
def order_by_nu(files,models,fits):
...
def conv_params(freq,cell,bmaj,bmin,bpa,beam,size_map,size_map_y,files,models,fits):
	...
	circ = False
	while track == False:
			prompt = raw_input('Do you want to restore them with the beam of one of them?\n')
			var = str(prompt)
			if var == 'y':
				track = True
				...
				index = int(prompt)
				while circ == False:			
					prompt = raw_input('Do you want to use the equivalent circular beam? (y/n)\n')
	...
	if len(positions) == len(freq):
		for i in xrange(0,len(positions)):
			files_out.append('CONV_ALL/'+source_name+'_'+str(int(round(freq2conv[i])))+'convolved_with_beam'+str('%1.2f' % (beam))+'and_cell'+str('%1.3f' % (pixelsize))+'.fits')
	else:
		for i in xrange(0,len(positions)):
			index = positions[i]
			files_out.append('CONV/'+source_name+'_'+str(int(round(freq2conv[i])))+'convolved_with_beam'+str('%1.2f' % (beam))+'and_cell'+str('%1.3f' % (pixelsize))+'.fits')


	#save convolution parameters to a pickle file
	final = [freq2conv,cell2conv,bmaj2conv,bmin2conv,bpa2conv,size_map2conv,size_map_y2conv,files2conv,models2conv,files_out,mapsize,pixelsize]

	if tofile > 1:
		res=open('convolve_param'+str(int(round(freq2conv[0])))+'to'+str(int(round(freq2conv[tofile])))+'.p','wb')
		pickle.dump(final,res)
		res.close()     
	else:
		res=open('convolve_param'+str(int(round(freq2conv[0])))+'and'+str(int(round(freq2conv[tofile])))+'.p','wb')
		pickle.dump(final,res)
		res.close() 
...
def check_map_params(image1,image2):
	"""v1 < v2"""

	header1 = take_header(image1)
	header2 = take_header(image2)
	...
	map_data1 = read_map(image1)		
	realDAT = map_data1[0]
	map_data2 = read_map(image2)		
